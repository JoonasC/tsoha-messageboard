...
CREATE TABLE message_chains (id SERIAL PRIMARY KEY, name VARCHAR(255), topic_id INTEGER REFERENCES topics(id), user_id INTEGER REFERENCES users(id), UNIQUE (topic_id, name));
...
CREATE TABLE message_replies (message_id INTEGER REFERENCES messages(id), replied_message_id INTEGER REFERENCES messages(id), UNIQUE (message_id, replied_message_id));
